...
Runtime secrets (`SUPABASE_URL`, `SUPABASE_KEY`) are **not** in the build output — set once on the Worker:
...
Local dev uses `.dev.vars` (not `.env`) for the workerd runtime.

### Supabase on production

In Supabase → **Authentication** → **URL Configuration**, set **Site URL** and **Redirect URLs** to your production Worker URL (see `context/changes/deployment/phase-3-record.md`).
...
List and roll back Worker **code only** (not Supabase schema/data):
...
Edge propagation can take a few minutes.
